...
class SelectMarkersDialog extends JDialog implements ActionListener
{
	...
	private JComboBox type, parent, ratio, dr;
	...
	private JPanel createControls()
	{
		...
		cType = new JCheckBox("Where the marker type is: ",
			Prefs.gui_select_on_type);
		...
		{
			case Marker.AFLP: type.setSelectedIndex(0); break;
			case Marker.SSR:  type.setSelectedIndex(1); break;
		}
		
		// Parent checkbox and combo items
		cParent = new JCheckBox("And the marker is present in: ",
			Prefs.gui_select_on_parent);
		...
		cRatio = new JCheckBox("Segregating with a ratio of: ",
			Prefs.gui_select_on_ratio);
		...
		{
			case Marker.R1_1:  ratio.setSelectedIndex(0); break;
			case Marker.R3_1:  ratio.setSelectedIndex(1); break;
			case Marker.R5_1:  ratio.setSelectedIndex(2); break;
			case Marker.R11_1: ratio.setSelectedIndex(3); break;
			case Marker.R35_1: ratio.setSelectedIndex(4); break;
		}
		
		// Ratio spinner controls
		ratioModel = new SpinnerNumberModel(Prefs.gui_select_ratio_sig,
			0, 1, 0.01);
		ratioSpin = new JSpinner(ratioModel);
		((JSpinner.NumberEditor)ratioSpin.getEditor()).getTextField()
			.setToolTipText("Minimum significance value that a marker will be "
				+ "considered to have this ratio at");
		ratioLabel = new JLabel("significance threshold: ");
		
		// Double reduction checkbox and combo items
		cDR = new JCheckBox("With double-reduction present: ",
			Prefs.gui_select_on_dr);
		cDR.addActionListener(this);
		dr = new JComboBox();
		dr.addItem("Yes");
		dr.addItem("No");
		if (!Prefs.gui_select_dr) dr.setSelectedIndex(1);
		
		// Double reduction spinner controls
		drModel = new SpinnerNumberModel(Prefs.gui_select_dr_sig,
			0, 1, 0.01);
		drSpin = new JSpinner(drModel);
		((JSpinner.NumberEditor)drSpin.getEditor()).getTextField()
			.setToolTipText("Maximum significance value that a marker will be "
	...
	private void selectMarkers()
	{
		...
		{
			case 0: Prefs.gui_select_type = Marker.AFLP; break;
			case 1: Prefs.gui_select_type = Marker.SSR; break;
		}
		switch (ratio.getSelectedIndex())
		{
			case 0: Prefs.gui_select_ratio = Marker.R1_1; break;
			case 1: Prefs.gui_select_ratio = Marker.R3_1; break;
			case 2: Prefs.gui_select_ratio = Marker.R5_1; break;
			case 3: Prefs.gui_select_ratio = Marker.R11_1; break;
			case 4: Prefs.gui_select_ratio = Marker.R35_1; break;
		}
